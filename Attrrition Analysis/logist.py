...
def mega(data):
    ...
    for i in range(size):
        ...
        res = y_intercept + y_coefficient[0][0] * (data.loc[i, 'satisfaction_level']) + y_coefficient[0][1] * (
        data.loc[i, 'last_evaluation']) + y_coefficient[0][2] * (data.loc[i, 'number_project']) + y_coefficient[0][
                  3] * (data.loc[i, 'average_montly_hours']) + y_coefficient[0][4] * (
              data.loc[i, 'time_spend_company']) + y_coefficient[0][5] * (data.loc[i, 'Work_accident']) + \
              y_coefficient[0][6] * (data.loc[i, 'promotion_last_5years']) + y_coefficient[0][7] * (
